    ...
    class ConfigCompat(SimpleNamespace):
        ...
        def __getattr__(self, name: str) -> Any:
            ...
            if not self._warned.get(name):
                logging.getLogger("compat").warning("[compat] optimizer config missing '%s' — using neutral default.", name)
...
def setup_logging():
    ...
    for name in ("alert", "telemetry", "main", "listener", "maker", "optimizer", "selector", "compat"):
...
class _DSAdapter:
    ...
    def __init__(self, source: Any, markets: Optional[List[str]] = None):
        self._src = source
        self._all_markets = markets or ["market:1", "market:2", "market:55", "market:99"]
    ...
    def _coerce_one(self, m: Any) -> Optional[SimpleNamespace]:
        ...
        if funding_apr is None and all(v is None for v in (funding_8h, funding_1h, funding_24h)):
    ...
    async def fetch_pair_metrics(self) -> List[SimpleNamespace]:
        ...
        if not top:
            ...
            top = [self._all_markets[idx], self._all_markets[(idx + 1) % len(self._all_markets)]]
...
class _MakerUpdater:
    ...
    def update_active_pairs(self, market_ids: Sequence[str]) -> None:
        if not self.maker:
            self.LOG.info("[updater] maker not present; ignoring pair update: %s", list(market_ids))
            return
        if not market_ids:
...
def _opt_cfg_from_dict(root_cfg: Dict[str, Any]) -> ConfigCompat:
    ...
    """
    src = (root_cfg.get("optimizer") or {}) if isinstance(root_cfg.get("optimizer"), dict) else {}
    ...
    floats = {
        "min_open_interest", "max_spread_bps", "w_funding", "w_oi",
        "spread_bps_penalty", "hysteresis_score_margin"
    }
...
async def main():
    ...
    fire_and_forget(alert_mgr.info("Startup", f"{app} starting (M7 Telemetry & Alerts)."))
    ...
    def handle_exception(loop, context):
        ...
        fire_and_forget(alert_mgr.error(
            "Unhandled exception",
            message=str(msg) if msg else repr(exc),
            fields={"type": type(exc).__name__ if exc else "unknown"} if exc else None,
        ))
    ...
    if MarketDataListener:
        listener = _try_construct(MarketDataListener, [
            ((), {"config": cfg, "state": state, "alert_manager": alert_mgr, "telemetry": telemetry}),
    ...
    if MockMetricsProvider:
        try: raw_ds = MockMetricsProvider()
        except Exception: raw_ds = None
    if not raw_ds and MockMetrics:
        try: raw_ds = MockMetrics()
        except Exception: raw_ds = None
    if not raw_ds:
    ...
    if FundingOptimizer:
        ...
        if sig and len(sig.parameters) >= 4:
            try:
                ...
                optimizer = _try_construct(FundingOptimizer, [
                    ((), {"config": cfg, "state": state, "alert_manager": alert_mgr, "telemetry": telemetry}),
                    ((), {"config": cfg, "state": state}),
                    ((cfg, state), {}),
                    ((state, cfg), {}),
    ...
    selector = None
    if PairSelector:
        selector = _try_construct(PairSelector, [
            ((), {"config": cfg, "state": state, "alert_manager": alert_mgr, "telemetry": telemetry}),
            ((), {"config": cfg, "state": state}),
            ((cfg, state), {}),
            ((state, cfg), {}),
    ...
    async def run_component(name: str, comp):
        ...
        if hasattr(comp, "run") and asyncio.iscoroutinefunction(comp.run):
            await comp.run(); return
        if hasattr(comp, "start") and asyncio.iscoroutinefunction(comp.start):
            await comp.start(); return
        if hasattr(comp, "start") and callable(comp.start):
            result = comp.start()
            if inspect.iscoroutine(result): await result
            elif isinstance(result, asyncio.Task): await result
            else: logging.getLogger("main").info("[%s] started (self-managed).", name); return
        logging.getLogger("main").info("[%s] no run/start; assuming self-managed.", name)

    if listener: tasks.append(asyncio.create_task(run_component("listener", listener)))
    if optimizer: tasks.append(asyncio.create_task(run_component("optimizer", optimizer)))
    if selector: tasks.append(asyncio.create_task(run_component("selector", selector)))
    if maker: tasks.append(asyncio.create_task(run_component("maker", maker)))
    ...
    async def watchdogs():
        ...
        logging.getLogger("main").info("[main] watchdogs enabled. ws_stale=%ss quote_stale=%ss", ws_stale_sec, quote_stale_sec)
        while not stop_event.is_set():
            now = time.time()
            _, _, hbs = telemetry.metrics.snapshot()
            ws_last = hbs.get("ws"); qt_last = hbs.get("quote")

            if ws_last:
                ws_age = now - ws_last
                if ws_age > ws_stale_sec and (now - last_ws_alert) > resend_every_sec:
                    last_ws_alert = now
                    fire_and_forget(alert_mgr.warning(
            ...
            if qt_last:
                q_age = now - qt_last
                if q_age > quote_stale_sec and (now - last_quote_alert) > resend_every_sec:
                    last_quote_alert = now
                    fire_and_forget(alert_mgr.warning(
                        "Maker quotes stale",
                        message=f"No quotes emitted for {int(q_age)}s.",
                        fields={"quote_last_ts": qt_last, "quote_age_s": int(q_age)},
                    ))
