    ...
    async def _maybe_hedge(self) -> None:
        ...
        if mid and self.exchange_min_notional > 0:
            ...
            if hedge_units < min_size_for_notional:
                ...
                hedge_units = max(hedge_units, Decimal(str(quantized)))
                LOG.debug(
                    "[hedger] clip quantization: adjusted from %.6f to %.6f (notional: %.2f @ %.2f = $%.2f)",
                    float(original_hedge_units),
                    float(hedge_units),
                    float(hedge_units),
                    mid,
        ...
        if hedge_units > abs_inv:
            hedge_units = abs_inv
            LOG.debug("[hedger] clip capped at inventory: %.4f", float(hedge_units))
