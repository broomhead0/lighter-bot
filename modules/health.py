...
class HealthMonitor:
    def __init__(
        self, max_silence_sec: int = 45, max_reconnects: int = 5, window_sec: int = 300
    ):
    ...
    def note_reconnect(self):
        ...
        self._reconnect_timestamps = [
            t for t in self._reconnect_timestamps if t >= cutoff
        ]
    ...
    def check(self):
        ...
        if now - self._last_msg_t > self.max_silence_sec:
            logger.warning(
                f"[HEALTH] No messages for {int(now - self._last_msg_t)}s (> {self.max_silence_sec}s)"
            )
            self._last_msg_t = now  # avoid spamming every tick

        if len(self._reconnect_timestamps) > self.max_reconnects:
            logger.warning(
                f"[HEALTH] Reconnects in last {self.window_sec}s = {len(self._reconnect_timestamps)} "
